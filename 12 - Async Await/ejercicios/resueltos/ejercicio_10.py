...
async def worker(nombre, cola):
    while True:
        pedido = await cola.get()
        async with ConexionBD():
            await asyncio.sleep(0.3)  # Simula procesamiento del pedido
            print(f"{nombre} procesó {pedido}")
        cola.task_done()
...
async def main():
    ...
    # Mete 6 pedidos en la cola
    for i in range(6):
        await cola.put(f"pedido_{i}")

    # Lanza 3 workers
    workers = [asyncio.create_task(worker(f"worker_{i}", cola)) for i in range(3)]
