"""
================================================================================
 EJERCICIOS: VARIABLES Y TIPOS DE DATOS EN PYTHON
...
# Crea: nombre_usuario (str), edad (int), altura_metros (float),
# es_premium (bool) y telefono (None, aún no lo ha rellenado)
nombre_usuario = ...
edad = ...
altura_metros = ...
es_premium = ...
...
# Imprime el tipo de cada variable con type()
...
# Calcula total (precio * cantidad) y total_con_iva (total + 21% de total)
# Redondea total_con_iva a 2 decimales con round()
total = ...
total_con_iva = ...
...
# Limpia los espacios con strip(), extrae el dominio ("api.ejemplo.com")
# usando split(), y comprueba si empieza por "https" con startswith()
url_limpia = ...
dominio = ...
es_https = ...
...
# Recorre la lista y construye un diccionario {producto: veces_vendido}
conteo = ...
...
# Lee "modo_oscuro" (por defecto False), "idioma" (por defecto "en") y
# "notificaciones" (por defecto True) usando SIEMPRE .get(), nunca []
modo_oscuro = ...
idioma = ...
notificaciones = ...
...
# Recorre valores_formulario con un bucle. Para cada texto, usa
# isdigit() para saber si es un entero, y texto.replace(".", "", 1).isdigit()
# para saber si es un decimal. Si no es ninguna de las dos cosas, añade None
resultados = []
...
...
# Filtra los valores válidos con texto.replace(".", "", 1).isdigit(),
# conviértelos a float, y calcula la media y el máximo de los válidos
precios_validos = ...
media_precios = ...
maximo_precio = ...
...
# Obtén de forma segura, con .get() encadenado y valores por defecto:
# el país del usuario (no existe -> "desconocido")
# el teléfono del usuario (no existe -> "no disponible")
pais = ...
telefono = ...
...
# Recorre valores con un bucle. Usa isinstance() para saber si cada
# valor es int, float o str: si lo es, conviértelo a float; si no,
# añade el texto "tipo no soportado" en su lugar
normalizados = []
...
...
# Calcula el porcentaje de éxito con sum() directamente sobre la lista
# de booleanos, y compáralo con una suma manual con un bucle
porcentaje_con_sum = ...
suma_manual = ...
